...
$conn = $database;
if(isset($_GET['id'])){
$qry = $conn->query("SELECT t.*,u.email as name FROM topics t inner join webuser u on u.email = t.user_id where t.id= ".$_GET['id']);
foreach($qry->fetch_array() as $k => $val){
	$$k=$val;
}
$comments = $conn->query("SELECT c.*,u.email as name,u.email as username FROM comments c inner join webuser u on u.email = c.user_id where c.topic_id= ".$_GET['id']." order by unix_timestamp(c.date_created) asc");
$com_arr= array();
while($row= $comments->fetch_assoc()){
	$com_arr[] = $row;
}
$replies = $conn->query("SELECT r.*,u.email as name,u.email as username FROM replies r inner join webuser u on u.email = r.user_id where r.comment_id in (SELECT id FROM comments where topic_id= ".$_GET['id'].") order by unix_timestamp(r.date_created) asc");
$rep_arr= array();
while($row= $replies->fetch_assoc()){
	$rep_arr[$row['comment_id']][] = $row;
}
if($user_id != $_SESSION['user']){
$chk = $conn->query("SELECT * FROM forum_views where  topic_id=$id and user_id='{$_SESSION['user']}' ")->num_rows;
if($chk <= 0){
	$conn->query("INSERT INTO forum_views set  topic_id=$id , user_id='{$_SESSION['user']}' ");
}
}
...
<div class="container-field">
	<div class="col-lg-12">
		<div class="card">
			<div class="card-body">
				<?php if($_SESSION['user'] == $row['user_id'] || $_SESSION['usertype'] == 'a'): ?>
    				...
    				<div>
    					...
    					<div class="col-lg-8 offset-lg-2 replies">
    						...
    						<div class="form-group ty-compact-list">
			                    <?php if($_SESSION['user'] == $rep['user_id']): ?>
			                    <div class="dropleft float-right">
			                      <a class="text-dark" href="javascript:void(0)" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			                        <span class="fa fa-ellipsis-v"></span>
			                      </a>
